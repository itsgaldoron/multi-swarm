...
#   HOME  — Base directory for IPC state storage ($HOME/.claude/multi-swarm/state/)
#
# Commands:
...
cmd_list() {
    ...
    for msg_file in "$messages_dir"/*.json; do
        ...
        local msg
        msg="$(cat "$msg_file")" || continue

        # Validate JSON before processing
        if ! echo "$msg" | jq empty 2>/dev/null; then
            echo "WARN: Skipping invalid JSON in $msg_file" >&2
            continue
        fi
        ...
        if [ -n "$filter_for" ]; then
            local msg_target
            msg_target="$(echo "$msg" | jq -r '.target')" || continue
        ...
        if [ -n "$filter_type" ]; then
            local msg_type
            msg_type="$(echo "$msg" | jq -r '.type')" || continue
        ...
        if [ -n "$filter_since" ]; then
            local msg_ts
            msg_ts="$(echo "$msg" | jq -r '.timestamp')" || continue
        ...
        if [ "$filter_unread" = true ]; then
            ...
            local msg_id
            msg_id="$(echo "$msg" | jq -r '.id')" || continue
    ...
    if [ ${#valid_messages[@]} -eq 0 ]; then
        echo "[]"
    else
        printf '%s\n' "${valid_messages[@]}" | jq -s 'sort_by(.timestamp)'
    fi
}
...
cmd_read() {
    ...
    # Validate msg_id format (8 lowercase alphanumeric chars)
    if [[ ! "$msg_id" =~ ^[a-z0-9]{8}$ ]]; then
        echo "ERROR: Invalid message ID format '$msg_id'" >&2
        exit 1
    fi
...
cmd_status() {
    ...
    for type in DISCOVERY BLOCKER BROADCAST REQUEST; do
        ...
        for msg_file in "$base/messages"/*.json; do
            [ -f "$msg_file" ] || continue
            local msg_type
            msg_type="$(jq -r '.type' "$msg_file" 2>/dev/null)" || continue
