...
CHECKPOINT_FILE="$STATE_DIR/merge-checkpoint.json"

save_checkpoint() {
    local swarm_num="$1"
    local status="$2"
    jq -n --arg num "$swarm_num" --arg status "$status" --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
        '{lastSwarm: $num, status: $status, timestamp: $ts}' > "$CHECKPOINT_FILE"
}

load_checkpoint() {
    if [ -f "$CHECKPOINT_FILE" ]; then
        log "Found checkpoint file, resuming from last successful merge..."
        local num
        for num in $(jq -r '.swarms | to_entries[] | select(.value.status == "merged") | .key' "$RESULTS_FILE" 2>/dev/null); do
            mark_merged "$num"
            log "  Restored: swarm $num (already merged)"
        done
    fi
}
...
merge_swarm() {
    ...
    # Use proper flock subshell pattern to avoid TOCTOU and FD leaks
    (
        flock -w 60 9 || {
        ...
        # Step 2: Push rebased branch (with retry)
        log "Pushing $branch..."
        if ! retry_with_backoff 3 2 git push origin "$branch" --force-with-lease 2>&1; then
        ...
        # Step 3: Create PR (with retry)
        log "Creating PR for swarm $swarm_num..."
        local pr_url
        pr_url=$(retry_with_backoff 3 2 gh pr create \
            --base "$BASE_BRANCH" \
            --head "$branch" \
...
EOF
        ...
        # Step 4: Squash merge and delete branch (with retry)
        log "Merging PR for swarm $swarm_num..."
        if ! retry_with_backoff 3 2 gh pr merge --squash --delete-branch 2>&1; then
        ...
        if ! git checkout "$BASE_BRANCH" 2>&1; then
            log_error "Failed to checkout $BASE_BRANCH after merge"
            exit 1
        fi
        if ! git pull origin "$BASE_BRANCH" 2>&1; then
            log_error "Failed to pull $BASE_BRANCH after merge"
            exit 1
        fi
    ...
    while true; do
        ...
        for swarm_num in $merge_order; do
            ...
            if merge_swarm "$swarm_num"; then
                mark_merged "$swarm_num"
                save_checkpoint "$swarm_num" "merged"
                log "Swarm $swarm_num merged ($(processed_count)/$SWARM_COUNT)"
            else
                mark_merged "$swarm_num"
                save_checkpoint "$swarm_num" "failed"
