...
#   WORKTREE_PATH     — Path to the new worktree (optional, default: current directory)
...
ENV_FILES=$(jq -r '.setup.envFilesToCopy // [".env", ".env.local", ".env.development.local"] | .[]' "$GLOBAL_CONFIG" 2>/dev/null) || ENV_FILES=$'.env\n.env.local\n.env.development.local'
