...
if ! command -v jq &>/dev/null; then
    echo "[quality-gate] ERROR: jq is required but not found. Install jq and retry." >&2
    exit 1
fi

log_error() { echo "[quality-gate] ERROR: $*" >&2; }
...
run_tests() {
    if [ -f "package.json" ]; then
        # Check for specific test scripts (single jq parse)
    ...
    if [ -f "package.json" ]; then
        ...
        if [ -n "$lint_files" ]; then
            if command -v npx &>/dev/null && [ -f ".eslintrc.js" ] || [ -f ".eslintrc.json" ] || [ -f ".eslintrc.yml" ] || [ -f "eslint.config.js" ] || [ -f "eslint.config.mjs" ]; then
                ...
                local staged_lint_cmd
                staged_lint_cmd=$(jq -r '.scripts.lint // empty' package.json 2>/dev/null)
                if [ -n "$staged_lint_cmd" ]; then
                    if [ -f "pnpm-lock.yaml" ]; then
                        pnpm lint 2>&1 || return 1
                    elif [ -f "yarn.lock" ]; then
                        yarn lint 2>&1 || return 1
