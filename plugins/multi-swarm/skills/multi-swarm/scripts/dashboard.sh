...
# Log helper
log_error() { echo "[dashboard] ERROR: $*" >&2; }
...
sparkline() {
    ...
    local sparks=("▁" "▂" "▃" "▄" "▅" "▆" "▇" "█")
    local max_idx=$(( ${#sparks[@]} - 1 ))
    for v in "${values[@]}"; do
        local idx=$((v * max_idx / max))
...
render_heatmap() {
    ...
    local tmpfile
    tmpfile=$(mktemp "${TMPDIR:-/tmp}/dashboard-heatmap.XXXXXX")
